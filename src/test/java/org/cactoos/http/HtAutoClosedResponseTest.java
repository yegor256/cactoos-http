...
final class HtAutoClosedResponseTest {
    ...
    void closesResponseAndThusSocketWhenEofIsReached() throws Exception {
        new FtRemote(new TkText("Hey")).exec(
            HtAutoClosedResponseTest::closesSocketOnEof
        );
    }

    /**
     * Reads a response and confirms the socket auto-closes on EOF.
     * @param home The address of the server
     * @throws IOException If fails
     */
    @SuppressWarnings("PMD.AvoidCatchingGenericException")
    private static void closesSocketOnEof(final URI home) throws IOException {
        try (Socket socket = new Socket(home.getHost(), home.getPort())) {
            try (
                InputStream ins = new HtAutoClosedResponse(
                    new HtResponse(
                        new HtWire(() -> socket),
                        new Get(home)
                    )
